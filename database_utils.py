...
def salvar_dados_cadastro(nome_aprendiz, dados_cadastro):
    ...
    if nome_aprendiz not in aprendizes: aprendizes[nome_aprendiz] = {}
...
def adicionar_nova_avaliacao(nome_aprendiz, nova_avaliacao):
    aprendizes = carregar_dados()
    if nome_aprendiz not in aprendizes: aprendizes[nome_aprendiz] = {}
    if "avaliacoes" not in aprendizes[nome_aprendiz]: aprendizes[nome_aprendiz]["avaliacoes"] = []
    aprendizes[nome_aprendiz]["avaliacoes"].append(nova_avaliacao)
    with open(DB_FILE, 'w', encoding='utf-8') as f:
        json.dump(aprendizes, f, ensure_ascii=False, indent=4)
    return True

def adicionar_plano_adaptacoes(nome_aprendiz, novo_plano):
    aprendizes = carregar_dados()
    if nome_aprendiz not in aprendizes: aprendizes[nome_aprendiz] = {}
    if "planos_adaptacao" not in aprendizes[nome_aprendiz]: aprendizes[nome_aprendiz]["planos_adaptacao"] = []
    aprendizes[nome_aprendiz]["planos_adaptacao"].append(novo_plano)
    with open(DB_FILE, 'w', encoding='utf-8') as f:
        json.dump(aprendizes, f, ensure_ascii=False, indent=4)
    return True
